...
def draw_node(vao, node, VP, MVP_loc, color_loc):
    MVP = VP * node.get_global_transform() * node.get_scale()
...
def main():
    ...
    arm = Node(base, glm.translate((0.2,0,.01)) * glm.scale((.25,.1,0.)), glm.vec3(1,0,0)) 
    arm_2 = Node(arm, glm.translate((0.2,0,0.02)) * glm.scale((.25,.1,0.)), glm.vec3(0,1,0))
    ...
    while not glfwWindowShouldClose(window):
        ...
        arm.set_transform(glm.translate((0, 0, 0)) * glm.rotate(t, (0,0,1)))
        arm_2.set_transform(glm.translate((0.5, 0, 0)) * glm.rotate(t, (0,0,1)))
        ...
        draw_node(vao_box, arm_2, P*V, MVP_loc_box, color_loc_box)
